...
ticktick_mcp.config validates credentials at import time and sys.exit(1)s when
any are missing, so placeholders are installed here - before any test imports
the package. Real credentials in the environment are left alone, which is what
the live tests need.
"""
...
def has_live_credentials() -> bool:
    """True when real TickTick credentials are configured, not placeholders."""
