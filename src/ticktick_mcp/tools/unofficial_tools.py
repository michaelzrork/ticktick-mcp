...
def _get_api_client() -> UnofficialAPIClient:
    ...
    Missing credentials and a login TickTick refused are different problems.
    Reporting both as "not configured" sends debugging after environment
    variables that were fine all along.
    """
    client = get_client()
    if not client:
